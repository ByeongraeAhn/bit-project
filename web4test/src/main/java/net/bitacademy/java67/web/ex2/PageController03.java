 ...
 *   ==> 귀찮다. ==> 해결책? 모든 페이지 컨트롤러에 적용할 수 있는 @InitBinder를 선언하면 된다.
 *   ==> 어떻게? ...web.ex2.GlobalInitBinder와 PageController4를 참고하라!
 * 
...
public class PageController03 {
  ...
  //1) 날짜 파라미터 값 받기
  // http://localhost:9999/web4/ex2/test11.do?today=2015-05-27 이렇게 들어가면 400에러가 뜬다.
  // 예외) The request sent by the client was syntactically incorrect.
  // 이유) yyyy-MM--dd 형식의 문자열을 java.util.Date 객체로 변환할 수 없어서 예외가 발생한다.
  // 해결책) 문자열로 된 요청 파라미터 값을 java.util.Date 객체로 변환하는 에디터를 등록해야 한다.
  //       Reference Documentation에 들어가서  Initbinder를 검색하여 추가한다.
  ...
  // request handler를 호출하기 전에 @InitBinder 에노테이션이 붙은 메서드가 먼저 호출된다.
  // @InitBinder를 붙일 수 있는 메서드의 형식
  // - 리턴 타입은 void
  // - 메서드명 마음대로
  // - 파라미터는 WebDateBinder 타입
  @InitBinder
  public void initBinder(WebDataBinder binder) {
      ...
      dateFormat.setLenient(false); // 문자열을 관대하게 해석하지 말라! 저 형식으로 들어오지 않으면 에러를 띄워라
